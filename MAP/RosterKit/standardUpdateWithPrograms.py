...
try:
    ...
    url = (
        ...
        + config.get('import types', 'standardUpdate')
    )
    username = config.get('mainconf', 'username')
    password = config.get('mainconf', 'password')
    programsPath = config.get('mainconf', 'programs_file')
...
#############################################################
    ...
    url = 'https://api.mapnwea.org/services/rostering/submit/complete/udpate'
    ...
    rosterPath = None
    # Replace 'None' witht he path to your programs file
    programPath = None
...
def submitRequest():
    ...
    with open(rosterPath, 'rb') as f:
        # Prepare the roster and programs files as an HTTP post object
        files = {'file': f, 'file1': open(programsPath, 'rb')}
