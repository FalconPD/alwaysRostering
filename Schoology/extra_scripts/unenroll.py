...
def main(db_file, environment, debug):
    """
    Unenrolls everyone for currently active courses
...
async def sync(loop, db_file, environment):
    """
    Performs all steps to perform Schoology unenrollments
    """
    ...
    async with schoology.Session(environment) as Schoology:
        async with Schoology.Enrollments as Enrollments:
            ...
            print(f"Unenrolling students and teachers in {len_sections} sections...")
            tasks = []
            for section in AR.sections():
                ...
                tasks.append(
                    Enrollments.delete(section_school_code, teacher_id, admin=True)
                )
            # enroll the students
                for student in section.active_students:
                    student_id = student.student_id
                    tasks.append(
                        Enrollments.delete(section_school_code, student_id)
                    )
